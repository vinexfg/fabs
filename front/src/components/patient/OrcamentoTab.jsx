...
const STATUS_LABELS = { rascunho: 'Rascunho', aprovado: 'Aprovado', recusado: 'Recusado' }
const STATUS_STYLES = {
...
const EMPTY_ITEM = { proc: '', dente: '', valor: '' }

export default function OrcamentoTab({ patientId, patient, clinic }) {
  ...
  function openEdit(budget) {
    setItems(budget.items.length ? budget.items.map(i => ({ ...i })) : [{ ...EMPTY_ITEM }])
      ...
      {budgets.length === 0
        ...
        : <div className={styles.list}>
            {budgets.map((budget, idx) => {
              const sub = (budget.items || []).reduce((s, i) => s + (parseFloat(i.valor) || 0), 0)
              const tot = Math.max(0, sub - (parseFloat(budget.desconto) || 0))
              return (
                  ...
                  <div className={styles.itemList}>
                    {(budget.items || []).slice(0, 3).map((item, i) => (
                      <div key={i} className={styles.itemRow}>
                        ...
                        <span className={styles.itemValor}>{fmtR(parseFloat(item.valor))}</span>
                      </div>
                    ))}
