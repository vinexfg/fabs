...
const OrcamentoPrint = React.forwardRef(function OrcamentoPrint({ patient, clinic, budget, numero }, ref) {
  ...
  const desconto = parseFloat(budget.desconto) || 0
        ...
        <tbody>
          {items.map((item, i) => (
            <tr key={i}>
              <td>{item.proc}</td>
              <td>{item.dente || '—'}</td>
              <td style={{ textAlign: 'right' }}>{fmtR(parseFloat(item.valor))}</td>
            </tr>
          ))}
        </tbody>
      </table>
