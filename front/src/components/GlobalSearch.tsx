...
import type { SearchResults } from '../infrastructure/http/SearchRepository'
...
const EMPTY_RESULTS: SearchResults = { patients: [], evolutions: [], appointments: [] }
...
export default function GlobalSearch() {
  ...
  const flat = [
    ...results.patients.map(p     => ({ type: 'patient' as const,     item: p, patientId: p.id })),
    ...results.evolutions.map(e   => ({ type: 'evolution' as const,   item: e, patientId: e.patientId })),
    ...results.appointments.map(a => ({ type: 'appointment' as const, item: a, patientId: a.patientId })),
  ]
  ...
  function go(patientId: string) { setOpen(false); navigate(`/pacientes/${patientId}`) }
