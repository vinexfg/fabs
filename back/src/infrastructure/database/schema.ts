...
export const hashPw = (password: string): string => bcrypt.hashSync(password, 10);

// Compatibilidade: verifica hash legado SHA-256 ou bcrypt
export const verifyPw = (password: string, stored: string): boolean => {
...
const runMigrations = (): void => {
  try { db.exec('ALTER TABLE patients ADD COLUMN foto TEXT'); } catch { /* coluna já existe */ }
  try { db.exec('ALTER TABLE patients ADD COLUMN anamnese TEXT'); } catch { /* coluna já existe */ }

  // Migra senha de SHA-256 para bcrypt se ainda não foi migrada
  const row = db.prepare('SELECT value FROM settings WHERE key = ?').get('password') as Setting | undefined;
...
const seedDefaultSettings = (): void => {
...
const seedDefaultTemplates = (): void => {
  const defaultTemplates: [string, string, number, string][] = [
